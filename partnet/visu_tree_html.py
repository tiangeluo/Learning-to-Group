...
def gen_html_for_tree_hier(html_fn, tree_hier, model_path, num):
    fout = open(html_fn, 'w')
    ...
    for level_id in range(max_level+1):
        ...
        for width_id in range(max_loc+1):
            if width_id in cur_level_locs_dict:
                cur_part_img = os.path.join('../png', num, str(cur_level_node_locs[width_id])+'.png')
                #cur_meta_file = os.path.join('./outputs/pn_mask_merge_purity_regress_hao/tree/txt', num, str(cur_level_node_locs[width_id])+'.txt')
                cur_meta_file = os.path.join(model_path, 'txt', num, str(cur_level_node_locs[width_id])+'.txt')
...
#model_path = '/home/luotg/shaper2/outputs/pn_mask_merge_purity_regress_hao/tree'
tree_hier_json = os.path.join(model_path, 'tree', num+'.json')

with open(tree_hier_json, 'r') as fin:
    tree_hier = json.load(fin)

html_fn = os.path.join(model_path, 'html', num+'.html')
gen_html_for_tree_hier(html_fn, tree_hier, model_path, num)
